...
SELECT Location AS País, date AS Data, total_cases AS CasosToTais, new_cases AS NovosCasos, total_deaths AS TotalMortes, population AS Populacao
FROM mortescovid
ORDER BY 1, 2
...
SELECT Location AS País, date AS Data, population AS Populacao, total_cases AS CasosTotais, (total_deaths/population)*100 AS PorcentagemInfectados
FROM mortescovid
...
SELECT Location AS País, date AS Data, total_cases AS CasosNovos, total_deaths AS TotalMortes, (total_deaths/total_cases)*100 AS PorcentagemMortes
FROM mortescovid
WHERE location = 'Brazil' and continent is not null
ORDER BY 1, 2
...
SELECT Location AS País, population AS Populacao, MAX(total_cases) AS MaiorinfestaçãoPorPais, MAX((total_cases/population))*100 AS PorcentagemInfectados
FROM mortescovid
...
SELECT Location AS País, MAX(cast(total_deaths as bigint)) AS MaximoTotalMortes
FROM mortescovid
WHERE continent is not null
GROUP BY location
...
SELECT continent AS Continente, MAX(cast(total_deaths as bigint)) AS MaximoTotalMortes
FROM mortescovid
WHERE continent is not null
GROUP BY continent
ORDER BY MaximoTotalMortes DESC
...
SELECT SUM(new_cases) as SomaCasosTotais, SUM(cast(new_deaths AS bigint)) AS QtdMortes, ROUND(SUM(cast(new_deaths AS bigint))/SUM(new_cases), 3)*100 AS PorcentagemMortesGlobal
FROM mortescovid
WHERE continent is not null
ORDER BY 1, 2
...
Select dea.continent AS Continente, dea.location AS País, dea.date AS Data, dea.population AS Populacao, vac.new_vaccinations AS NovasVacinacoes
